...
function PreviewBanner() {
  return (
    <div className="border-b border-amber-200 bg-amber-50/80 px-5 py-2.5 text-center text-xs text-amber-900 sm:px-8">
      Preview mode — this report is in your browser only and won&apos;t persist if you close the tab. To save reports and email them automatically, set <code className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-[11px]">SUPABASE_SERVICE_ROLE_KEY</code> and <code className="rounded bg-amber-100 px-1.5 py-0.5 font-mono text-[11px]">RESEND_API_KEY</code>.
    </div>
  )
}
...
function SleeperBreakdown({ report }: { report: AuditReport }) {
  ...
  const buckets = [
    { label: 'Light sleepers (14–20d)', n: r.sleepers.light, body: 'Habit at risk. A friendly check-in here is enough.', tone: 'bg-amber-200' },
    { label: 'Deep sleepers (21–45d)', n: r.sleepers.deep, body: 'Habit broken, still salvageable. Highest-leverage band.', tone: 'bg-orange-300' },
    { label: 'Critical (46–60d)', n: r.sleepers.critical, body: 'Last chance. Manual call only.', tone: 'bg-red-300' },
    { label: 'Lost (60d+)', n: r.sleepers.lost, body: 'Do not contact — outreach may trigger cancellation.', tone: 'bg-zinc-300' },
  ]
